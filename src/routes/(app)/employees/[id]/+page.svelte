...
>
	...
	<form class="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm" method="POST" action="?/updateProfile">
		<div class="grid gap-4 md:grid-cols-2">
			<label class="space-y-1 text-sm">
				...
				<input name="name" required value={data.employee.name} class="w-full rounded-md border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500" />
			</label>
			<label class="space-y-1 text-sm">
				<span class="text-slate-700">Type</span>
				<select name="type" value={data.employee.type} class="w-full rounded-md border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500">
			...
			<label class="space-y-1 text-sm">
				...
				<select name="status" value={data.employee.status} class="w-full rounded-md border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500">
			...
			<label class="space-y-1 text-sm">
				...
				<input type="date" name="startDate" value={data.employee.startDate ?? ''} class="w-full rounded-md border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500" />
			</label>
			<label class="space-y-1 text-sm">
				<span class="text-slate-700">End Date</span>
				<input type="date" name="endDate" value={data.employee.endDate ?? ''} class="w-full rounded-md border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500" />
			</label>
			<label class="space-y-1 text-sm">
				<span class="text-slate-700">Contact</span>
				<input name="contact" value={data.employee.contact ?? ''} class="w-full rounded-md border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500" />
			</label>
			<label class="space-y-1 text-sm md:col-span-2">
				<span class="text-slate-700">Tax ID</span>
				<input name="taxId" value={data.employee.taxId ?? ''} class="w-full rounded-md border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500" />
			</label>
		</div>

		<div class="flex gap-3">
			<button class="rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-700" type="submit">Save Profile</button>
	...
	<section class="space-y-3 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
		...
		<form class="grid gap-3 md:grid-cols-5" method="POST" action="?/addSalary">
			...
			<button class="rounded bg-slate-900 px-3 py-2 text-sm text-white hover:bg-slate-700 md:col-span-5" type="submit">Add Salary Record</button>
		</form>
