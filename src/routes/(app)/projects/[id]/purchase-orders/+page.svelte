...
<PageShell
	...
	title={`${data.project.name} - Purchase Order Management`}
	...
	<div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
		<table class="min-w-full divide-y divide-slate-200 text-sm">
			<thead class="bg-slate-50 text-left text-slate-600">
				<tr><th class="px-4 py-3">PO Number</th><th class="px-4 py-3">Supplier</th><th class="px-4 py-3">Amount</th><th class="px-4 py-3">Date</th><th class="px-4 py-3">Actions</th></tr>
			</thead>
			<tbody class="divide-y divide-slate-100">
				{#if data.purchaseOrders.length === 0}
					<tr><td class="px-4 py-6 text-slate-500" colspan="5">No purchase order records yet.</td></tr>
				{:else}
