	...
	<form method="GET" class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
		<div class="flex flex-wrap items-end gap-3">
			...
			<a
				...
				href={`/api/finance/reports/projects-profit/export?projectId=${encodeURIComponent(data.filters.projectId)}&projectStatus=${encodeURIComponent(data.filters.status)}&from=${encodeURIComponent(data.filters.from)}&to=${encodeURIComponent(data.filters.to)}`}
				target="_blank"
				rel="noreferrer"
			>
				Export CSV
