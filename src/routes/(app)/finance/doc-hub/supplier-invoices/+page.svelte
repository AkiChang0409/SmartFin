<script lang="ts">
	...
	type SupplierInvoiceItem = (typeof data.invoices)[number];
	let selectedInvoice = $state<SupplierInvoiceItem | null>(null);

	const openDetail = (item: SupplierInvoiceItem) => {
...
<PageShell
	...
	description="Archive and review supplier invoice records. OCR actions will be enhanced in phase 2."
>
	<section class="space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
		<form class="grid gap-3 lg:grid-cols-[2fr_1fr_1.3fr_auto_auto]" method="GET" data-sveltekit-noscroll>
			...
			<a class="inline-flex h-10 items-center justify-center rounded border border-[var(--sf-gold)] bg-[var(--sf-gold-soft)] px-4 text-center text-sm font-medium text-[#7a5a07] hover:bg-[#f6e8b8] lg:mt-6" href="/finance/doc-hub/supplier-invoices" data-sveltekit-noscroll>
				Reset
			</a>
		</form>
					...
					<table class="min-w-full divide-y divide-slate-200 text-xs">
						<thead class="bg-slate-50 text-left text-slate-600">
							<tr>
								...
								<th class="px-3 py-2">Supplier Invoices</th>
						...
						<tbody class="divide-y divide-slate-100">
							{#each data.projects as project}
								<tr class={data.filters.projectId === project.id ? 'bg-[var(--sf-green-soft)]/50' : ''}>
									...
									<td class="px-3 py-2">{project.supplierInvoiceCount}</td>
									<td class="px-3 py-2">
										<a
											...
											href={`/finance/doc-hub/supplier-invoices?q=${encodeURIComponent(data.filters.q)}&status=${encodeURIComponent(data.filters.status)}&startedAfter=${encodeURIComponent(data.filters.startedAfter)}&page=${data.filters.page}&projectId=${encodeURIComponent(project.id)}&listMode=selected&supplierQ=${encodeURIComponent(data.filters.supplierQ)}&supplierField=${encodeURIComponent(data.filters.supplierField)}`}
				...
				<div class="mt-3 flex items-center justify-between gap-3 text-xs text-slate-600">
					...
					<div class="flex items-center gap-2">
						{#if data.pagination.hasPrev}
							<a class="rounded border border-slate-300 bg-white px-2 py-1 hover:bg-slate-100" href={`/finance/doc-hub/supplier-invoices?q=${encodeURIComponent(data.filters.q)}&status=${encodeURIComponent(data.filters.status)}&startedAfter=${encodeURIComponent(data.filters.startedAfter)}&page=${data.pagination.page - 1}&listMode=${encodeURIComponent(data.filters.listMode)}&supplierQ=${encodeURIComponent(data.filters.supplierQ)}&supplierField=${encodeURIComponent(data.filters.supplierField)}`} data-sveltekit-noscroll>
								Previous
							</a>
						{/if}
	...
	<section class="space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
		<form class="grid gap-3 md:grid-cols-[1fr_180px_180px_auto]" method="GET" data-sveltekit-noscroll>
			...
			<input class="rounded border border-slate-300 px-3 py-2 text-sm" name="supplierQ" value={data.filters.supplierQ} placeholder="Filter supplier invoice list: ID / supplier / amount / date / status / PO" />
			<select class="rounded border border-slate-300 px-3 py-2 text-sm" name="supplierField">
				<option value="all" selected={data.filters.supplierField === 'all'}>All Fields</option>
				<option value="id" selected={data.filters.supplierField === 'id'}>Invoice ID</option>
				<option value="supplier" selected={data.filters.supplierField === 'supplier'}>Supplier</option>
				<option value="amount" selected={data.filters.supplierField === 'amount'}>Amount</option>
				<option value="date" selected={data.filters.supplierField === 'date'}>Invoice Date</option>
				<option value="status" selected={data.filters.supplierField === 'status'}>Status</option>
				<option value="poNumber" selected={data.filters.supplierField === 'poNumber'}>PO Number</option>
			</select>
			<select class="rounded border border-slate-300 px-3 py-2 text-sm" name="listMode">
				<option value="all" selected={data.filters.listMode === 'all'}>All Supplier Invoices</option>
		...
		<table class="min-w-full divide-y divide-slate-200 text-sm">
			...
			<tbody class="divide-y divide-slate-100">
				{#if data.filters.listMode === 'selected' && !data.filters.projectId}
					<tr><td class="px-4 py-6 text-slate-500" colspan="8">Select a project to view supplier invoices.</td></tr>
				{:else if data.invoices.length === 0}
					<tr><td class="px-4 py-6 text-slate-500" colspan="8">No supplier invoices found.</td></tr>
				{:else}
		...
		<div class="fixed inset-0 z-50 flex items-center justify-center p-4">
			...
			<div class="relative max-h-[85vh] w-full max-w-4xl overflow-auto rounded-xl border border-slate-200 bg-white p-5 shadow-xl">
				<div class="flex items-start justify-between gap-3">
					<div>
						<p class="text-xs font-semibold uppercase tracking-wide text-[var(--sf-green)]">Supplier Invoice Detail</p>
						<p class="mt-1 text-lg font-semibold text-slate-900">{selectedInvoice.supplierName ?? selectedInvoice.id}</p>
				...
				<div class="mt-4 grid gap-3 md:grid-cols-2">
					...
					<div class="rounded border border-slate-200 bg-slate-50 p-3"><p class="text-xs text-slate-500">PO Number</p><p class="mt-1 text-sm text-slate-800">{selectedInvoice.poNumber ?? '--'}</p></div>
					<div class="rounded border border-slate-200 bg-slate-50 p-3"><p class="text-xs text-slate-500">Invoice Date / Due Date</p><p class="mt-1 text-sm text-slate-800">{selectedInvoice.invoiceDate ?? '--'} / {selectedInvoice.dueDate ?? '--'}</p></div>
					<div class="rounded border border-slate-200 bg-slate-50 p-3"><p class="text-xs text-slate-500">Amount / GST / Currency</p><p class="mt-1 text-sm text-slate-800">{selectedInvoice.amount} / {selectedInvoice.gstAmount} / {selectedInvoice.currency}</p></div>
					...
					<div class="rounded border border-slate-200 bg-slate-50 p-3"><p class="text-xs text-slate-500">Confidence</p><p class="mt-1 text-sm text-slate-800">{selectedInvoice.ocrConfidence ? `${(selectedInvoice.ocrConfidence * 100).toFixed(1)}%` : '--'}</p></div>
				</div>

				{#if selectedInvoice.rawParsed}
					<div class="mt-3 rounded border border-slate-200 bg-slate-50 p-3">
						<p class="text-xs text-slate-500">OCR Parsed Payload</p>
						<pre class="mt-2 max-h-56 overflow-auto rounded border border-slate-200 bg-white p-2 text-xs text-slate-600">{JSON.stringify(selectedInvoice.rawParsed, null, 2)}</pre>
					</div>
				{/if}
