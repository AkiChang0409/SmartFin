<script lang="ts">
	...
	type ContractItem = (typeof data.contracts)[number];
	let selectedContract = $state<ContractItem | null>(null);
...
<PageShell
	eyebrow="AR / Contracts"
	title="Contract Management"
	description="Archive and review contract records. Edit actions are handled in project module."
>
				...
				<div class="mt-2 overflow-hidden rounded-lg border border-slate-200 bg-white">
					<table class="min-w-full divide-y divide-slate-200 text-xs">
						<thead class="bg-slate-50 text-left text-slate-600">
							<tr>
								...
								<th class="px-3 py-2">Contracts</th>
					...
					{#each data.projects as project}
							<tr class={data.filters.projectId === project.id ? 'bg-[var(--sf-green-soft)]/50' : ''}>
								...
								<td class="px-3 py-2">{project.contractCount}</td>
								<td class="px-3 py-2">
									<a
										...
										href={`/finance/doc-hub/contracts?q=${encodeURIComponent(data.filters.q)}&status=${encodeURIComponent(data.filters.status)}&startedAfter=${encodeURIComponent(data.filters.startedAfter)}&page=${data.filters.page}&projectId=${encodeURIComponent(project.id)}&listMode=selected&contractQ=${encodeURIComponent(data.filters.contractQ)}&contractField=${encodeURIComponent(data.filters.contractField)}`}
										data-sveltekit-noscroll
									>
										View Contracts
									</a>
	...
	<section class="space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
		<div class="flex flex-wrap items-end gap-3">
			<form class="grid flex-1 gap-3 md:grid-cols-[1fr_180px_180px_auto]" method="GET" data-sveltekit-noscroll>
				...
				<input
					class="w-full rounded border border-slate-300 px-3 py-2 text-sm"
					name="contractQ"
					value={data.filters.contractQ}
					placeholder="Filter contract list: ID / amount / date / notes"
				/>
				<select class="w-full rounded border border-slate-300 px-3 py-2 text-sm" name="contractField">
					<option value="all" selected={data.filters.contractField === 'all'}>All Fields</option>
					<option value="id" selected={data.filters.contractField === 'id'}>Contract ID</option>
					<option value="amount" selected={data.filters.contractField === 'amount'}>Amount</option>
					<option value="date" selected={data.filters.contractField === 'date'}>Date</option>
					<option value="notes" selected={data.filters.contractField === 'notes'}>Notes</option>
				</select>
				<select class="w-full rounded border border-slate-300 px-3 py-2 text-sm" name="listMode">
					<option value="all" selected={data.filters.listMode === 'all'}>All Contracts</option>
					<option value="selected" selected={data.filters.listMode === 'selected'}>Selected Project</option>
					<option value="unassigned" selected={data.filters.listMode === 'unassigned'}>Unassigned Only</option>
				</select>
				<button class="rounded border border-[var(--sf-green)] bg-[var(--sf-green)] px-4 py-2 text-sm font-medium text-white hover:bg-[#2f5e2c]" type="submit">
					Filter List
				</button>
			</form>
		</div>
		...
		<table class="min-w-full divide-y divide-slate-200 text-sm">
			...
			<tbody class="divide-y divide-slate-100">
				{#if data.filters.listMode === 'selected' && !data.filters.projectId}
					<tr><td class="px-4 py-6 text-slate-500" colspan="7">Select a project to view contract records.</td></tr>
				{:else if data.contracts.length === 0}
					<tr><td class="px-4 py-6 text-slate-500" colspan="7">No contract records in this project.</td></tr>
				{:else}
					{#each data.contracts as item}
						<tr class="align-top">
							<td class="px-4 py-3">
								<p class="font-medium text-slate-800">{item.projectName}</p>
								<p class="text-xs text-slate-500">{item.projectId ?? 'NO_PROJECT'}</p>
							</td>
							<td class="px-4 py-3">{item.effectiveDate ?? '--'}</td>
							...
							<td class="px-4 py-3">
								<div class="flex flex-wrap gap-2">
									<button
										type="button"
										class="rounded border border-slate-300 bg-white px-2 py-1.5 text-xs text-slate-700 hover:bg-slate-100"
									...
									<a
										...
										href={item.projectId ? `/projects/${item.projectId}/documents/contracts/${item.id}` : '/projects'}
										data-sveltekit-noscroll
									>
		...
		<div class="fixed inset-0 z-50 flex items-center justify-center p-4">
			...
			<div class="relative max-h-[85vh] w-full max-w-3xl overflow-auto rounded-xl border border-slate-200 bg-white p-5 shadow-xl">
				<div class="flex items-start justify-between gap-3">
					<div>
						<p class="text-xs font-semibold uppercase tracking-wide text-[var(--sf-green)]">Contract Detail</p>
						<p class="mt-1 text-lg font-semibold text-slate-900">{selectedContract.projectName}</p>
						<p class="text-xs text-slate-500">{selectedContract.projectId}</p>
					</div>
					<button type="button" class="rounded border border-slate-300 px-2 py-1 text-xs text-slate-600 hover:bg-slate-100" onclick={closeDetail}>
						Close
					</button>
				</div>
				...
				<div class="mt-4 grid gap-3 md:grid-cols-2">
					<div class="rounded border border-slate-200 bg-slate-50 p-3">
						...
						<p class="mt-1 break-all text-sm text-slate-800">{selectedContract.id}</p>
					</div>
					<div class="rounded border border-slate-200 bg-slate-50 p-3">
						<p class="text-xs text-slate-500">Date</p>
						<p class="mt-1 text-sm text-slate-800">{selectedContract.effectiveDate ?? '--'}</p>
					</div>
					<div class="rounded border border-slate-200 bg-slate-50 p-3">
						<p class="text-xs text-slate-500">Amount / Currency</p>
						<p class="mt-1 text-sm text-slate-800">{selectedContract.amount ?? 0} {selectedContract.currency ?? 'SGD'}</p>
					</div>
					<div class="rounded border border-slate-200 bg-slate-50 p-3">
						<p class="text-xs text-slate-500">Source Type</p>
						<p class="mt-1 text-sm text-slate-800">{selectedContract.docMeta?.sourceType ?? 'manual'}</p>
					</div>
					<div class="rounded border border-slate-200 bg-slate-50 p-3">
						<p class="text-xs text-slate-500">Parse Status</p>
						<p class="mt-1 text-sm text-slate-800">{selectedContract.docMeta?.parseStatus ?? 'not_parsed'}</p>
					</div>
					<div class="rounded border border-slate-200 bg-slate-50 p-3">
						<p class="text-xs text-slate-500">Uploaded At</p>
						<p class="mt-1 text-sm text-slate-800">{selectedContract.docMeta?.upload?.uploadedAt ?? '--'}</p>
					</div>
				</div>

				<div class="mt-3 rounded border border-slate-200 bg-slate-50 p-3">
					<p class="text-xs text-slate-500">Document</p>
					{#if selectedContract.fileViewUrl}
						<a class="mt-1 inline-block break-all text-sm text-[var(--sf-green)] hover:underline" href={selectedContract.fileViewUrl} target="_blank" rel="noreferrer">
							{selectedContract.docMeta?.upload?.fileName ?? selectedContract.fileUrl}
						</a>
					{:else}
				...
				<div class="mt-3 rounded border border-slate-200 bg-slate-50 p-3">
					...
					<p class="mt-1 whitespace-pre-wrap text-sm text-slate-800">{selectedContract.docMeta?.notes || '--'}</p>
				</div>

				<div class="mt-4 flex justify-end">
					{#if selectedContract.projectId}
						<a class="rounded bg-[var(--sf-green)] px-3 py-2 text-sm text-white hover:bg-[#2f5e2c]" href={`/projects/${selectedContract.projectId}/documents/contracts/${selectedContract.id}`} data-sveltekit-noscroll>
