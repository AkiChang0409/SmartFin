<script lang="ts">
	...
	type QuotationItem = (typeof data.quotations)[number];
	let selectedQuotation = $state<QuotationItem | null>(null);

	const openDetail = (item: QuotationItem) => {
...
<PageShell
	...
	description="Archive and review quotation records. Edit actions are handled in project module."
>
						...
						<tbody class="divide-y divide-slate-100">
							{#each data.projects as project}
								<tr class={data.filters.projectId === project.id ? 'bg-[var(--sf-green-soft)]/50' : ''}>
									...
									<td class="px-3 py-2">{project.quotationCount}</td>
									<td class="px-3 py-2">
										<a
											...
											href={`/finance/doc-hub/quotations?q=${encodeURIComponent(data.filters.q)}&status=${encodeURIComponent(data.filters.status)}&startedAfter=${encodeURIComponent(data.filters.startedAfter)}&page=${data.filters.page}&projectId=${encodeURIComponent(project.id)}&listMode=selected&quotationQ=${encodeURIComponent(data.filters.quotationQ)}&quotationField=${encodeURIComponent(data.filters.quotationField)}`}
											data-sveltekit-noscroll
										>
											View Quotations
										</a>
		...
		<div class="flex flex-wrap items-end gap-3">
			<form class="grid flex-1 gap-3 md:grid-cols-[1fr_180px_180px_auto]" method="GET" data-sveltekit-noscroll>
				...
				<input
					class="w-full rounded border border-slate-300 px-3 py-2 text-sm"
					name="quotationQ"
					value={data.filters.quotationQ}
					placeholder="Filter quotation list: ID / amount / date / source / notes"
				/>
				<select class="w-full rounded border border-slate-300 px-3 py-2 text-sm" name="quotationField">
					<option value="all" selected={data.filters.quotationField === 'all'}>All Fields</option>
					<option value="id" selected={data.filters.quotationField === 'id'}>Quotation ID</option>
					<option value="amount" selected={data.filters.quotationField === 'amount'}>Amount</option>
					<option value="date" selected={data.filters.quotationField === 'date'}>Date</option>
					<option value="source" selected={data.filters.quotationField === 'source'}>Source Type</option>
					<option value="notes" selected={data.filters.quotationField === 'notes'}>Notes</option>
				</select>
				<select class="w-full rounded border border-slate-300 px-3 py-2 text-sm" name="listMode">
					<option value="all" selected={data.filters.listMode === 'all'}>All Quotations</option>
		...
		<table class="min-w-full divide-y divide-slate-200 text-sm">
			...
			<tbody class="divide-y divide-slate-100">
				{#if data.filters.listMode === 'selected' && !data.filters.projectId}
					<tr><td class="px-4 py-6 text-slate-500" colspan="7">Select a project to view quotation records.</td></tr>
				{:else if data.quotations.length === 0}
					<tr><td class="px-4 py-6 text-slate-500" colspan="7">No quotation records found.</td></tr>
				{:else}
					{#each data.quotations as item}
						<tr class="align-top">
							...
							<td class="px-4 py-3">{item.quotationNumber ?? 'manual'}</td>
							<td class="px-4 py-3">
								{#if item.fileViewUrl}
							...
							<td class="px-4 py-3">
								<div class="flex flex-wrap gap-2">
									...
									<a
										...
										href={item.projectId ? `/projects/${item.projectId}/documents/quotations/${item.id}` : '/projects'}
										data-sveltekit-noscroll
									>
		...
		<div class="fixed inset-0 z-50 flex items-center justify-center p-4">
			...
			<div class="relative max-h-[85vh] w-full max-w-3xl overflow-auto rounded-xl border border-slate-200 bg-white p-5 shadow-xl">
				<div class="flex items-start justify-between gap-3">
					<div>
						<p class="text-xs font-semibold uppercase tracking-wide text-[var(--sf-green)]">Quotation Detail</p>
						<p class="mt-1 text-lg font-semibold text-slate-900">{selectedQuotation.projectName}</p>
						<p class="text-xs text-slate-500">{selectedQuotation.projectId}</p>
					</div>
				...
				<div class="mt-4 grid gap-3 md:grid-cols-2">
					<div class="rounded border border-slate-200 bg-slate-50 p-3">
						...
						<p class="mt-1 break-all text-sm text-slate-800">{selectedQuotation.id}</p>
					</div>
					<div class="rounded border border-slate-200 bg-slate-50 p-3">
						<p class="text-xs text-slate-500">Date</p>
						<p class="mt-1 text-sm text-slate-800">{selectedQuotation.date ?? '--'}</p>
					</div>
					<div class="rounded border border-slate-200 bg-slate-50 p-3">
						<p class="text-xs text-slate-500">Amount / Currency</p>
						<p class="mt-1 text-sm text-slate-800">{selectedQuotation.amount ?? 0} {selectedQuotation.currency ?? 'SGD'}</p>
					</div>
					<div class="rounded border border-slate-200 bg-slate-50 p-3">
						<p class="text-xs text-slate-500">Source Type</p>
						<p class="mt-1 text-sm text-slate-800">{selectedQuotation.quotationNumber ?? 'manual'}</p>
					</div>
					<div class="rounded border border-slate-200 bg-slate-50 p-3">
						<p class="text-xs text-slate-500">Parse Status</p>
						<p class="mt-1 text-sm text-slate-800">{selectedQuotation.docMeta?.parseStatus ?? 'not_parsed'}</p>
					</div>
					<div class="rounded border border-slate-200 bg-slate-50 p-3">
						<p class="text-xs text-slate-500">Uploaded At</p>
						<p class="mt-1 text-sm text-slate-800">{selectedQuotation.docMeta?.upload?.uploadedAt ?? '--'}</p>
					</div>
				</div>

				<div class="mt-3 rounded border border-slate-200 bg-slate-50 p-3">
					<p class="text-xs text-slate-500">Document</p>
					{#if selectedQuotation.fileViewUrl}
						<a class="mt-1 inline-block break-all text-sm text-[var(--sf-green)] hover:underline" href={selectedQuotation.fileViewUrl} target="_blank" rel="noreferrer">
							{selectedQuotation.docMeta?.upload?.fileName ?? selectedQuotation.fileUrl}
						</a>
					{:else}
				...
				<div class="mt-3 rounded border border-slate-200 bg-slate-50 p-3">
					...
					<p class="mt-1 whitespace-pre-wrap text-sm text-slate-800">{selectedQuotation.docMeta?.notes || '--'}</p>
				</div>

				<div class="mt-4 flex justify-end">
					{#if selectedQuotation.projectId}
						<a class="rounded bg-[var(--sf-green)] px-3 py-2 text-sm text-white hover:bg-[#2f5e2c]" href={`/projects/${selectedQuotation.projectId}/documents/quotations/${selectedQuotation.id}`} data-sveltekit-noscroll>
