...
>
	{#snippet actions()}
		<a
			...
			href={data.filters.projectId
				? `/finance/customer-invoices/generate?projectId=${encodeURIComponent(data.filters.projectId)}`
				: '/finance/customer-invoices/generate'}
	...
	<section class="space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
		<form class="grid gap-3 lg:grid-cols-[2fr_1fr_1.3fr_auto_auto]" method="GET" data-sveltekit-noscroll>
			...
			<a class="inline-flex h-10 items-center justify-center rounded border border-[var(--sf-gold)] bg-[var(--sf-gold-soft)] px-4 text-center text-sm font-medium text-[#7a5a07] hover:bg-[#f6e8b8] lg:mt-6" href="/finance/customer-invoices" data-sveltekit-noscroll>
				Reset
			</a>
		</form>
								...
								<div class="flex flex-wrap gap-2">
									...
									{#if item.status === 'draft'}
										<a
											...
											href={`/finance/customer-invoices/generate?invoiceId=${encodeURIComponent(item.id)}`}
											data-sveltekit-noscroll
										>
											Re-edit Draft
										</a>
