...
>
	...
	<form method="POST" class="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
		<div class="grid gap-3 md:grid-cols-4">
			...
			<div class="md:col-span-2 flex items-end">
				<a
					...
					href={`/finance/tax?year=${data.year}&quarter=${data.quarter}`}
				>
					Back to Tax
				</a>
			</div>
		...
		<div class="flex flex-wrap gap-3">
			...
			<a class="rounded-md border border-slate-300 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50" href={`/finance/tax/settings?year=${data.year}&quarter=${data.quarter}`}>
				Reload
			</a>
		</div>
	</form>
