...
// ---------------------------------------------------------------------------
// Chip option catalogues — surfaced to the step components for bucket / kind /
...
export type KindOption = {
	...
	label: string;
	sub?: string;
	/** For expense kinds only — the expense_type column value. */
	expenseType?: 'opex' | 'sales_cost';
	/** For expense kinds — the doc_type column value. */
...
export function getFieldSpecs(ctx: Ctx): FieldSpec[] {
	...
	// --- Expense ---------------------------------------------------------
	if (bucket === 'expense') {
		// Sales cost — supplier invoice/receipt tied to a project deliverable
		...
		if (category === 'allowance') {
			// Pure manual — no file. Still goes through intake so the staff UX is uniform.
			return [
		...
		// others / unknown — stick to the common fields
