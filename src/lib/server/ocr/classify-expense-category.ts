...
Decision rules:
- sales_cost categories are ONLY "invoice" or "receipt" — they represent a supplier bill (or its payment confirmation) for project-linked work.
...
- Map by merchant + content:
    * Grab / Uber / taxi / Gojek / MRT → opex.transport, doc_type: receipt
    * Hotel / Airbnb / Booking.com → opex.accommodation, doc_type: receipt or invoice
    * Restaurant / cafe / food delivery → opex.meal, doc_type: receipt
    * Gift shop / florist / FTD → opex.gift, doc_type: receipt
    * Cash per-diem with no merchant → opex.allowance, doc_type: null
    * ChatGPT / OpenAI / Cursor / Anthropic / Cloudflare / AWS / Vercel / GitHub / Figma / Notion → opex.ai_subscription, doc_type: invoice
    * DHL / FedEx / SingPost / UPS / SF Express → opex.logistics, doc_type: receipt or invoice
    * Electronic components / BOM / bulk procurement with PO number → opex.purchase, doc_type: po
    * Supplier tax invoice for a project deliverable → sales_cost.invoice, doc_type: invoice
    * Supplier payment receipt for same → sales_cost.receipt, doc_type: receipt
...
function heuristicFallback(
	...
	// First-pass says invoice_in → default to sales_cost.invoice (most common case)
	...
	// expense (receipt) with no specific signal → opex.others
	...
	try {
		...
		if (json && typeof json === 'object' && !Array.isArray(json)) {
			...
			if (expenseType === 'sales_cost' && category !== 'invoice' && category !== 'receipt') {
				return { ...fallback, reason: 'model picked sales_cost with opex category — kept heuristic' };
			}
		...
		// LLM call failed — heuristic fallback is fine.
	}
