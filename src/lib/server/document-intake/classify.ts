/**
 * Classify layer — decides the (bucket, docType, category) triple that
 * drives which extractor runs next.
 *
 * Two sequential LLM passes:
 *   1. classifyDocType         → 7-way docType
 *   2. classifyExpenseCategory → (expense only) refines to 11-way category
 *
 * The classifier is imperfect (see user feedback on sales_cost.invoice
 * being mis-classified as opex.logistics) — the ReviewStep's Re-check
...
/**
 * Combined classify — runs docType classifier, then (for expense bucket)
