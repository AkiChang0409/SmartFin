 ...
 *
 * Each extractor returns Partial<ExtractedFields> — only the keys relevant
 * to its document shape. ReviewStep on the client picks the subset it
 * needs per spec (see field-specs.ts). Keeping one superset type avoids a
...
export interface ExtractedFields {
	// Common — invoice/receipt/PO/contract/quotation all share these
	...
	// Reference numbers — populated per doc shape
