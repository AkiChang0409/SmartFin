 ...
 *   1. Classify  — decide (bucket, docType, category) from rawText
 *   2. Extract   — directed, category-specific LLM prompt
 *   3. Match     — find candidate projects for this doc
 *   4. Narrate   — one-liner for the UI
 *
 * Two entry points:
 *   • runIntakePipeline      — fresh intake, classifier decides everything
 *   • runReclassifyPipeline  — user forced (bucket, kind), skip classifier
 *
 * Future layers that will slot in here without restructuring:
 *   • Container unpacking (zip / eml / docx / xlsx) — before (1)
 *   • Page selection — between normalise and (1) for multi-page docs
 *   • Cost tiering (cheap regex → LLM fallback) — inside (2)
 *   • Confidence banding — wraps (2)'s output
 */
 ...
 * step-3 chips or the review pills. We skip classifyIntake entirely —
 * the forced pair IS the answer — and just re-run extract + match +
...
export async function runReclassifyPipeline(
	...
	return {
		...
		fields,
		projectMatches,
		// User-forced → by definition the source of truth; display as 100.
		confidence: 100,
		narration,
		provider: {
