 ...
 *   • transport      — Grab / taxi / MRT receipt
 *   • meal           — restaurant / cafe receipt
 *   • accommodation  — hotel / lodging receipt
 *   • gift           — gift shop / florist receipt
 *   • logistics      — courier / shipping receipt (trackingNumber key)
 *   • sales_cost     — formal payment receipt from a supplier
 *   • others         — generic business expense receipt
 */
...
Context: Transport expense receipt (Grab / Gojek / Uber / taxi / MRT / Comfort Delgro / ride-hail). supplierName = the ride-hail app or transport provider. trackingNumber is NOT relevant here — leave null.`;
	}
	if (variant === 'meal') {
		return `${common}
...
Context: Courier / shipping / logistics receipt (DHL / FedEx / SingPost / UPS / SF Express / Ninja Van). supplierName = the courier. **trackingNumber is a KEY field** — look for "Tracking No.", "AWB", "Waybill", "Consignment No."`;
	}
