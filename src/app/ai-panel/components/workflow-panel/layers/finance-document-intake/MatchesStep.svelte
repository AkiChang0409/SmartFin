<script lang="ts">
	...
	import { advanceWorkflow, type MatchingResult } from '$app-layer/ai-panel/workflow/finance-workflow-api';
	...
	function pickSupplier(id: string) { panel.patchState({ selectedSupplierId: id }); }
	function pickPo(id: string) { panel.patchState({ selectedPoId: id }); }
	function clearPo() { panel.patchState({ selectedPoId: null }); }

	function onContinue() {
		// Project step handles the next server advance.
		panel.advanceStep();
	}
...
{#if stage === 'loading'}
	<div class="loading">
		...
		<div class="loading-sub">Supplier directory 路 open POs 路 duplicate check</div>
	</div>
{:else if stage === 'error'}
	...
	<div class="step">
		...
		<section class="block">
			<header class="block-header">
				<h3 class="block-title">Counterparty</h3>
				<span class="block-sub">Top {matching.supplierCandidates.length} candidates</span>
			</header>

			{#if matching.supplierCandidates.length === 0}
				<div class="empty">No matching counterparty in directory. Confirm step will treat this as new.</div>
			{:else}
				<ul class="cards">
						...
						<li>
							...
							>
								<span class="row-main">
									...
									<span class="row-meta">{candidate.recentInvoiceCount} recent records</span>
								</span>
		...
		<section class="block">
			<header class="block-header">
				...
				<span class="block-sub">
					...
					{:else}
						None found 路 standalone record
					{/if}
				</span>
			</header>
			...
			{:else}
						...
						<li>
							...
							>
								<span class="row-main">
									...
									<span class="row-meta">
										{candidate.supplierName} 路 {formatAmount(candidate.totalAmount, candidate.currency)}
									</span>
								</span>
								<span class="row-aside">
									<span class="score">{pct(candidate.matchScore)}%</span>
				...
				{#if selectedPoId}
					<button type="button" class="link-btn" onclick={clearPo}>No PO for this record</button>
				{/if}
			{/if}
		</section>

		<div class="actions">
			<button type="button" class="btn-ghost" onclick={onBack}>鈫?Review</button>
			<button
				type="button"
				class="btn-primary"
				onclick={onContinue}
...
<style>
	...
	.ripple { display: inline-flex; gap: 10px; }
	.ripple-dot {
	...
	.ripple-dot:nth-child(2) { animation-delay: 0.18s; }
	.ripple-dot:nth-child(3) { animation-delay: 0.36s; }
	@keyframes breathe {
		0%, 100% { opacity: 0.3; transform: scale(0.85); }
		50% { opacity: 1; transform: scale(1.1); }
	}
	.loading-title { font-size: 16px; font-weight: 500; color: var(--panel-fg); }
	.loading-sub {
		font-size: 11.5px;
	...
	.dup-title { font-size: 13px; font-weight: 500; color: var(--panel-fg); }
	.dup-sub { font-size: 12.5px; color: var(--panel-fg-muted); line-height: 1.5; }

	.block { display: flex; flex-direction: column; gap: 10px; }
	.block-header { display: flex; align-items: baseline; gap: 10px; }
	.block-title { font-size: 14px; font-weight: 500; color: var(--panel-fg); margin: 0; }
	.block-sub {
		font-size: 11px;
		color: var(--panel-fg-faint);
	...
	.row:hover { border-color: var(--panel-gold); background: var(--panel-surface-raised); }
	.row.is-selected {
	...
	.row-aside { display: inline-flex; align-items: center; gap: 8px; flex-shrink: 0; }
	.score {
		font-size: 11.5px;
		font-variant-numeric: tabular-nums;
	...
	.link-btn:hover { color: var(--panel-gold-bright); }
	...
	.btn-ghost:hover, .btn-secondary:hover { color: var(--panel-fg); border-color: var(--panel-gold); }
	.btn-primary {
		background: var(--panel-gold);
	...
	.err-title { font-size: 17px; font-weight: 500; color: var(--panel-fg); }
	.err-sub {
		font-size: 13px;
		line-height: 1.55;
		color: var(--panel-fg-muted);
