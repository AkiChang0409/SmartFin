<script lang="ts">
	...
	const categoryId = $derived(wfState.selectedCategoryId as string | undefined);
	...
	function buildPayload(): (ConfirmedPayload & { categoryId?: string }) | null {
	...
	async function onConfirm() {
		...
		if (!payload || !serverWorkflowId) {
			errorMessage = 'Missing data â€?go back to the upload step.';
			stage = 'error';
			return;
		}
		stage = 'submitting';
		...
		// Server expects the user_confirmation step before /confirm; advance there first.
		try {
			const { advanceWorkflow } = await import('$app-layer/ai-panel/workflow/finance-workflow-api');
			await advanceWorkflow(serverWorkflowId, { targetStep: 'user_confirmation' });
		} catch {
			// If we're already at user_confirmation or the advance is idempotent, ignore.
		}
		...
		try {
			...
			errorMessage = e instanceof Error ? e.message : 'Could not record the document.';
			stage = 'error';
		}
	}
	...
	<div class="step">
		<div class="lede">
			...
			<span class="lede-text">
				About to record this document. Confirmation locks it into
		...
		<div class="summary-card">
			<dl class="summary">
				...
				<div class="row">
					<dt>Matched PO</dt>
					<dd>{po?.poNumber ?? 'â€?no PO link'}</dd>
				</div>
				<div class="row">
					<dt>Project</dt>
					<dd>{selectedProjectId ?? 'â€?company overhead'}</dd>
				</div>
			</dl>
				...
				{#if errorIssues.length}
					<ul class="err-issues">
						{#each errorIssues as issue, i (i)}
							<li><b>{issue.field}</b> â€?{issue.message}</li>
						{/each}
					</ul>
				{/if}
			</div>
...
{:else}
	<div class="empty">Missing extraction data. Step back through the flow.</div>
{/if}
...
<style>
	...
	.warn-icon { display: inline-flex; color: var(--panel-danger); }
	...
	.err-issues { list-style: disc; padding-left: 22px; margin: 8px 0 0; font-size: 12px; color: var(--panel-fg-muted); }
	.err-issues :global(b) { color: var(--panel-fg); font-weight: 500; }
